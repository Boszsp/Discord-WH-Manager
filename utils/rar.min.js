(function () {
  ...
  Reader.prototype.open = function (file, callback) {
    ...
    switch (this.type) {
      ...
      case Reader.OPEN_FILE:
        ...
        break;
      default:
        this.ajax({uri: this.file, type: "HEAD"}, function (err, resp, xhr) {
          if (err) {
            return callback(err);
          }
  ...
  Reader.prototype.ajax = function (opts, callback) {
    var options = {type: "GET", uri: null, responseType: "text"};
  ...
  Reader.prototype.readUri = function (length, position, callback) {
    this.ajax({uri: this.file, type: "GET", responseType: "arraybuffer", range: [position, position + length - 1]}, function (err, buffer) {
      if (err) {
        return callback(err);
      }
    ...
    for (var i = offset; i < offset + length; i += 2) {
      ...
      if ((ch >= 0 && ch <= 55295) || (ch >= 57344 && ch <= 65535)) {
        ...
        if (useBuffer) {
          ...
          str += String.fromCharCode(((1047552 & ch) >> 10) + 55296) + String.fromCharCode((1023 & ch) + 56320);
        }
      }
    }
    if (useBuffer) {
  ...
  RarEntry.METHOD_STORE = 48;
  RarEntry.METHOD_FASTEST = 49;
  RarEntry.METHOD_FAST = 50;
  RarEntry.METHOD_NORMAL = 51;
  RarEntry.METHOD_GOOD = 52;
  RarEntry.METHOD_BEST = 53;
  var RarArchive = function () {
    if (!(this instanceof RarArchive)) {
      return new RarArchive(arguments[0], arguments[1]);
    }
    this.options = {type: RarArchive.OPEN_URI};
  ...
  RarArchive.prototype.validate = function (callback) {
    ...
    this.rd.open(this.file, function (err) {
      ...
      self.rd.read(14, 0, function (err, data) {
        ...
        if (view.getString(7, 0, true) !== "Rar!\x00") {
  ...
  RarArchive.prototype.parseEntry = function (offset, callback) {
    ...
    this.rd.read(11, offset, function (err, data) {
      ...
      self.rd.read(size, offset, function (err, data) {
        ...
        entry.partial = (flags & 1) !== 0 || (flags & 2) !== 0;
        entry.continuesFrom = (flags & 1) !== 0;
        entry.continues = (flags & 2) !== 0;
        entry.offset = offset;
        ...
        entry.encrypted = (flags & 4) !== 0;
        var nameSize = view.getUint16(26, true);
        if ((flags & 256) !== 0) {
          entry.sizePacked += view.getUint32(32, true) * 4294967296;
          entry.size += view.getUint32(36, true) * 4294967296;
          entry.path = view.getString(nameSize, 40);
        } else {
          entry.path = view.getString(nameSize, 32);
        }
        if ((flags & 512) !== 0 && entry.path.indexOf("\x00") !== -1) {
