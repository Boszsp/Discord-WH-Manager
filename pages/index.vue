        ...
        <div class="flex flex-col gap-6 md:pr-4">
          ...
          <div class="embed-editors">
            <div class="mt-2" v-for="(_, i) in hookJson.embeds">
              <EmbedEditor @delete="(id)=>{hookJson.embeds.splice(id,1);console.log(id)}" :id="i" v-model="hookJson.embeds[i]" />
            </div>
            <div class="mt-6">
              <v-btn @click="addEmbed" color="primary" size="small" variant="flat">Add Embed</v-btn>
            </div>
          </div>
        </div>
      </v-col>
