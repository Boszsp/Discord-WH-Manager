...
function fetching() {
  ...
  isError.value = false;
  if (props?.method?.toUpperCase() == "POST" || props?.method?.toUpperCase() == "PATCH" || props?.method?.toUpperCase() == "DELETE") {
    $fetch(murl.value, {
...
<template>
  <v-card class="rounded-lg bg-component-background" :elevation="2" rounded :loading="isLoading">
    <div :class="'px-4 pt-2 pb-8 flex flex-col lg:flex-row gap-4 border-l-4 ' + (props?.method?.toUpperCase() == 'DELETE' ? 'border-danger' : props?.method?.toUpperCase() == 'POST' || props?.method?.toUpperCase() == 'PATCH' ? 'border-warning' : 'border-success')">
      <span class="flex flex-col gap-4 lg:w-1/2">
        ...
        <v-textarea rows="3" label="body" v-model="body" color="primary" density="compact" variant="outlined" hide-details class="bg-component-background"></v-textarea>
      </span>
      ...
      <span class="lg:w-1/2">
        ...
        <div class="flex gap-4">
          <span>
            <v-chip :color="isError ? 'danger' : responseM ? 'success' : 'warning'">{{ props?.method?.toUpperCase() ?? "GET" }} Status</v-chip>
          </span>
